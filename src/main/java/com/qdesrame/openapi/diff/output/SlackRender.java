...
public class SlackRender implements Render {
  ...
  public static final Logger LOGGER = LoggerFactory.getLogger(SlackRender.class);

  protected static RefPointer<Schema> refPointer = new RefPointer<>(RefType.SCHEMAS);
  protected final String H3 = "⚫️";
  ...
  public String render(ChangedOpenApi diff) {
    this.diff = diff;
    return listEndpoints("🎉 *ᴡʜᴀᴛ's ɴᴇᴡ*", diff.getNewEndpoints())
        + listEndpoints("🧨 *ᴡʜᴀᴛ's ᴅᴇʟᴇᴛᴇᴅ*", diff.getMissingEndpoints())
        + listEndpoints("🗑 *ᴡʜᴀᴛ's ᴅᴇᴘʀᴇᴄᴀᴛᴇᴅ*", diff.getDeprecatedEndpoints())
  ...
  protected String itemEndpoint(String method, String path, String summary) {
    return H5 + CODE + method + " " + path + CODE + "\n\n" + metadata(summary) + "\n";
  }
  ...
  protected String listEndpoints(List<ChangedOperation> changedOperations) {
    ...
    StringBuilder sb = new StringBuilder(sectionTitle("⚠️ *ᴡʜᴀᴛ's ᴄʜᴀɴɢᴇᴅ*"));
    changedOperations
        .stream()
        .map(
            operation -> {
              ...
              if (result(operation.getParameters()).isDifferent()) {
                details
                    .append(titleH5("_*Parameters:*_"))
                    .append(parameters(operation.getParameters()));
              }
              if (operation.resultRequestBody().isDifferent()) {
                details
                    .append(titleH5("_*Request:*_"))
              ...
              if (operation.resultApiResponses().isDifferent()) {
                details
                    .append(titleH5("_*Return Type:*_"))
